...
/*fprintf(stderr,"fread_header: ssave allocated\n");*/
  xvhips_getline(fd,&ssave[0],&slmax[0]);
  ...
  while(*(s += strlen(s)-3) == '|') {
    ...
    xvhips_getline(fd,&ssave[lineno],&slmax[lineno]);
    s = ssave[lineno];
  }
  len += strlen(ssave[lineno]);
  ...
  lineno = 0;
  len = 1;
  while(strcmp(xvhips_getline(fd,&ssave[lineno],&slmax[lineno]),".\n")) {
    len += strlen(ssave[lineno]);
    lineno++;
    if (lineno >= LINES)
...
static char *xvhips_getline(fd,s,l)
  int fd;
  char **s;
  int *l;
{
